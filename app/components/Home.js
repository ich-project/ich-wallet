...
class Home extends Component<Props> {
	...
	async getAccountData() {
	    ...
	    this.currentBlock = await tronWeb.trx.getCurrentBlock();
		...
		const transactions = await request.get(
			`https://apilist.tronscan.org/api/transaction?sort=-timestamp&count=true&address=${address}`
		);
	...
	componentWillMount() {
		this.getAccountData();
		setInterval(this.updateBlockHeight,30000);
			...
			onConfirm: async () => {
				if (values.token === "TRX") {
					try {
						...
						console.log("ICH", sendTransaction);
					} catch (err) {
						alert(err);
					}
				} else {
		...
		try {
			...
			this.setState({
				unfreezeBalance: {
					...
					error: true,
					txt:
						"Unable to unfreeze ICH. This could be caused because the minimal freeze period hasn't been reached yet."
				}
			});
		}
	}
			...
			return (
				<div className={styles.container}>
					...
					<Row className="mb-4">
						...
						<Col xs="4">
							<figure className={`${styles.box} ${styles.dark}`}>
								<h2 className={styles.title}>ICH Power</h2>
								<p className="text-sm">
									ICH can be frozen to gain ICH Power and
									enable additional features. For example,
									with ICH Power you can vote for Super
									Representatives.You can gain bandwith or
									energy as well. Frozen tokens are "locked"
									for a period of 3 days. During this period
									the frozen ICH cannot be traded. After this
									period you can unfreeze the ICH and trade
									...
									vote using the remaining ICH Power, he will
										...
										<Form>
											...
											<Field
												...
												items={[
													{
														key:
															"Freeze ICH to gain bandwith",
														value: 0
													},
													{
														key:
															"Freeze ICH to gain energy",
														value: 1
													}
												]}
												// ich={tronWeb.fromSun(balance)}
												component={InputOptions}
											/>
					...
					>
						...
						{(unfreezeBalance && unfreezeBalance.txID && (
							...
							<div>
								...
								<p>Are you sure you want to unfreeze ICH?</p>
								<Formik
									onSubmit={(values, { setSubmitting }) => {
										...
										<Form>
											<Field
												...
												items={[
													{
														key:
															"Unfreeze the bandwidth account to get ICH",
														value: 0
													},
													{
														key:
															"Unfreeze the energy account to get ICH",
														value: 1
													}
												]}
												// ich={tronWeb.fromSun(balance)}
